...
Run `jarvis.mjs pack` first and build on its output; that half is deterministic and costs nothing, so regenerating it by hand is waste. Add only what a command cannot know: which files actually matter for THIS request, and why. Report locations and constraints; draw no conclusions and propose no design — the moment this agent starts explaining causes it has become research-orchestrator at a lower tier.

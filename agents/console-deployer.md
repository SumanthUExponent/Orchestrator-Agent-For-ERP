...
**You own exactly this.** Idempotent, re-runnable System Console installers. On managed hosting this is the ONLY deployment path — bench does not exist there.
...
**Skills to load first.** `console-automation-engine` · `console-report-engineer`

These carry the actual expertise. Load them before reasoning about the task; do not reconstruct their content from memory.

**Constraints.**

safe_exec applies to every embedded script body. Never shorten an installer at the cost of idempotency, dependency order, or the safe_exec-mandated forms.

...
export function install({ root, readYaml, apply = false, external = false, force = false, target = skillsDir() }) {
  ...
  if (apply) {
    ...
    if (fs.existsSync(orchDest)) {
      // tests/fixtures too: the outcome probes grade against them, and without them
      // `outcome` reported 3 of 4 model-free probes as FAILING from the installed copy
      // when the truth was "fixture absent". Small, static, and the whole point of a
      // known-answer probe is that the answer travels with it.
      for (const dir of ['scripts', 'registry', 'tests/fixtures']) {
...
// 4. agents — inert until they reach the agents dir, which is separate from skills
  ...
  // Ghosts in the INSTALLED agents directory.
  //
  // `buildAgents` cleans the repo's own agents/ dir when an agent is renamed. Nothing
  // cleaned the installed one -- so a rename left the old agent live on the machine, and
  // the roster the model can actually select from silently diverged from the registry.
  //
  // Found the hard way: four pre-rebrand agents (frappe-architect, frappe-backend,
  // frappe-data, frappe-frontend) were still installed and routable alongside their
  // renamed replacements. A research pass reported that as ambiguous ownership; I checked
  // the repo, found nothing, and called it a stale-name error. It was checking the wrong
  // directory -- the ghosts were real, and they were mine.
  //
  // Only files this repo GENERATES are candidates: the marker line every generated agent
  // carries. A hand-written agent someone else put there is not ours to delete.
  const ghosts = [];
  if (fs.existsSync(agentsTo) && agentFiles.length) {
    const known = new Set(agentFiles);
    for (const f of fs.readdirSync(agentsTo).filter((x) => x.endsWith('.md'))) {
      if (known.has(f)) continue;
      let body = '';
      try { body = fs.readFileSync(path.join(agentsTo, f), 'utf8'); } catch { continue; }
      if (!body.includes('GENERATED from registry/agents.yaml')) continue;
      ghosts.push(f);
      if (apply) fs.rmSync(path.join(agentsTo, f), { force: true });
    }
  }

  return { target, agentsTarget: agentsTo, planned, skipped, written, agentsWritten, ghosts, applied: apply, selfContained };
}
...
export function render(result) {
  ...
  if (result.ghosts && result.ghosts.length) {
    console.log(`\n${result.applied ? 'Removed' : 'Would remove'} (${result.ghosts.length}) — installed but no longer in the registry`);
    for (const g of result.ghosts) console.log(`  - ${g.padEnd(28)} generated by this repo, since renamed or deleted`);
    console.log('  A renamed agent left live on the machine is selectable, so the roster the');
    console.log('  model sees diverges from the registry — silently, and in the direction of MORE.');
  }

...
// NOTE: this module deliberately does NOT import jarvis.mjs. The CLI
...
function repoCategories(routing, cwd) {
  ...
  for (const [glob, categories] of pats) {
    // Convert via a sentinel. Replacing **/ first and * second re-mangled the
    // first replacement's own output: "(?:.*/)?" contains a literal *, so it
    // became "(?:.[^/]*/)?" and never matched a nested path.
    const SENT = '\u0000';   // escaped, NOT a raw NUL byte: a raw one makes git and
                          // grep treat this whole file as binary, so every sweep
                          // skips it silently -- which is how it kept a stale name
    const re = new RegExp(
      '^' +
        glob
          .replace(/[.+^${}()|[\]\\]/g, '\\$&')
...
export function plan(reg, request, opts = {}) {
  ...
  // Cheapest signal first, most expensive last — a request carrying both a question
  // opener and an architecture noun ("explain how we'd migrate the whole platform")
  // is architecture work, so `full` must be able to overrule `fast`.
  let effort = opts.effort || 'standard';
  if (!opts.effort) {
    if ((routing.complexity.fast?.signals || []).some((s) => has(q, s))) effort = 'fast';
...
export function route(reg, request, opts = {}) {
  if (!String(request || '').trim()) {
    console.log('usage: jarvis.mjs route "<request>"');
    return 2;
  }

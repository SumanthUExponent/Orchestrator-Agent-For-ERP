...
```bash
ORCH=~/.claude/skills/jarvis          # or the repo root, if working from a clone

node $ORCH/scripts/jarvis.mjs route "<the user's request>"   # explain a routing decision
node $ORCH/scripts/jarvis.mjs health                         # validate the ecosystem
```
...
```bash
node scripts/jarvis.mjs build     # from the repo root
```
